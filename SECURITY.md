# 安全策略

cnb-agentic-memory (cam) 是基于 CNB 平台的智能体记忆工具，涉及 CNB API 凭据管理与智能体记忆读写。我们高度重视安全问题。

## 支持版本

仅最新稳定版本接收安全补丁，不对旧版本进行回溯修复。

| 版本 | 支持状态 |
| --- | --- |
| 最新稳定版 | ✅ |
| < 最新 | ❌ |

请查看 [Releases](https://cnb.cool/xqitw/cnb-agentic-memory/-/releases) 获取当前版本信息。

## 报告漏洞

**请勿通过公开 Issue 报告安全漏洞。**

请通过以下方式私下报告：

- **CNB 私信**：在仓库页面发起私密讨论，标题注明 `[SECURITY]`
- **邮件**：发送至项目维护者，主题行包含 `[SECURITY]`

报告中请包含：

- 漏洞描述及被利用后的影响
- 复现步骤或概念验证代码
- 受影响的版本和配置信息
- 建议的缓解方案（可选）

## 响应时间线

| 阶段 | 目标时间 |
| --- | --- |
| 确认接收 | 收到报告后 48 小时内 |
| 评估分类 | 5 个工作日内 — 评估严重程度、影响范围和修复方案 |
| 补丁发布（严重） | 评估完成后 14 天内 |
| 补丁发布（中等/低危） | 下一个功能版本发布或与维护者协商的时间线 |
| 公开披露 | 补丁发布且用户有充分时间更新后 |

## 安全设计原则

- **凭据安全**：CNB 访问令牌通过环境变量（`CAM_TOKEN`）传入，绝不写入代码、日志或文档；工具对用户请求中的令牌类内容应拒绝存储（见 MCP 客户端交互规范）。注意区分：`CAM_TOKEN` 是本工具的配置项（`CAM_` 前缀，见 README/CLI 文档），`CNB_TOKEN` 是 CNB 平台 CI 流水线运行时自动注入的变量，两者语境不同，不要把 CNB 平台令牌误配给本工具
- **最小权限**：令牌仅需 `repo-issue:rw`（Issue 读写）+ `repo-code:r`（知识库检索），文档明确告知用户所需最小权限集
- **软删除优先**：记忆删除为 Issue 归档（state=closed），保留完整审计链（activities），无物理删除
- **输入校验**：记忆编号、标签命名严格校验，非法输入返回清晰错误
