...
- 错误输出到 stderr，退出码非 0：
  - 退出码 2：配置错误或参数/用法错误（配置缺失判据见下条，以 stderr 文案为准）
  - 退出码 1：API 错误（含状态码与 CNB 响应原文，截断至 500 字符）/ 业务错误（如写入回读校验失败，含已落盘分片编号）
  - 退出码 70：内部错误（设置 `CNB_AGENTIC_MEMORY_DEBUG=1` 重试可看完整堆栈）
- **配置缺失时不要猜测连接参数、不要自行重试**：判据是 stderr 出现「缺少必需配置：」
  （退出码 2 但也可能是参数/用法错误，以文案为准），此时把缺失的环境变量清单转达
  给用户，由用户完成配置后重试。仅此场景不重试；写操作失败仍按下一条恢复阶梯处理。
  凭据（Token、仓库 slug）只能来自用户、环境变量或 MCP 请求头，Agent 不得编造或代填。
