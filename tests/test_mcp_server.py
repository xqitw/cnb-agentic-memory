...
def test_config_missing_raises_toolerror(monkeypatch, caplog):
    """配置缺失走 _tool_guard 出口：raise ToolError（协议级 isError）+ warning 留痕。

    框架对 ToolError 返回 is_error=True + 全文 content（server.py 实测）——
    若返回裸 str 则 isError 为 False，配置缺失被客户端当成功。文案与
    SKILL.md/CLI 判据三方同源（见 tests/test_guidance.py 运行时锚点）。
    """
    import asyncio
    ...
    with caplog.at_level("WARNING", logger=mcp_server.__name__):
        with pytest.raises(ToolError) as exc_info:
            asyncio.run(tool.fn(number=1))

    message = str(exc_info.value)
    assert "缺少必需配置：" in message
    assert "CNB_AGENTIC_MEMORY_TOKEN" in message
    assert "CNB_AGENTIC_MEMORY_REPO" in message
    # warning 留痕：静默吞没会让长驻进程"工具报错、服务端无痕"，排障无线索
    assert any("配置问题" in rec.message for rec in caplog.records)

    # 协议级锚定：下沉到 _handle_call_tool（is_error=True 转换的发生层）——
    # 只断言工具层 raise 锁不住「上游改 return is_error=False」的静默失效。
    from types import SimpleNamespace

    from mcp_types import CallToolRequestParams

    async def call_via_protocol():
        # stdio 形态：request 为 None → ctx.headers 为 None
        rc = SimpleNamespace(request=None, request_id="proto-1")
        return await mcp._handle_call_tool(
            rc, CallToolRequestParams(name="memory_get", arguments={"number": 1})
        )

    result = asyncio.run(call_via_protocol())
    assert result.is_error is True  # 协议级 isError
    text = "".join(getattr(c, "text", "") for c in result.content)
    assert "Error executing tool memory_get" in text  # 框架包装形态
    assert "缺少必需配置：" in text  # 指引文案未丢
...
def test_shared_client_pool_cross_loop_isolated(
    ...
    # loop1：绑定池（asyncio.run 结束即关——正好模拟临时 loop 劫持后死亡）
    ...
    # 已绑定存活 loop 后，模拟异 loop 访问：假 loop 探针（不真跑事件循环）
    class _ForeignLoop:
        @staticmethod
        def is_closed() -> bool:
            return False
    ...
    pool._loop = foreign  # type: ignore[assignment]
...
def test_shared_client_pool_dead_loop_self_heal(
    ...
    async def scenario():
        ...
        # 模拟绑定 loop 死亡：不真关（同 loop 内不能关自己），直接把 _loop 换成
        # 一个已关闭的假 loop，探活应识别并解绑重绑到当前存活 loop
        class _DeadLoop:
            @staticmethod
            def is_closed() -> bool:
                return True

        pool._loop = _DeadLoop()  # type: ignore[assignment]
        caplog.clear()
        c2 = await pool.acquire()
        assert any("解绑并重建绑定" in r.message for r in caplog.records)
        assert pool._loop is asyncio.get_running_loop()
        assert c2 is not c1  # 死条目已作废，同键重建新实例
