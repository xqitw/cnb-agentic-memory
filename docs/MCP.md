...
- **错误处理**：工具失败以协议级 `isError` 标记；`memory_search` 的错误文本保留 CNB 原始信息（状态码/原因），其余工具的错误被框架包装为笼统的 `Error executing tool <名称>`，具体状态码与原因需查服务端日志——据此可得信息决策重试、换参数或放弃。仓库未配置知识库流水线时 `memory_search` 会失败，错误文本提示可改用 `memory_keyword_search`（标题检索，无需知识库）。**例外——配置缺失不要重试**：错误为协议级 `isError` 且文案含「缺少必需配置」（凭据未提供或提供不齐，可能来自服务端环境变量或请求头，视部署形态而定）时，把缺失项清单转达给用户并等其完成配置后重试——不要猜测连接参数，不要编造或代填凭据
