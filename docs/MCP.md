...
| `--allowed-host` | `CNB_AGENTIC_MEMORY_MCP_ALLOWED_HOSTS` | 无 | DNS rebinding 防护额外放行的 Host 白名单（可多次传入或逗号分隔）。反代按最佳实践保留真实 Host（`proxy_set_header Host $host`）部署时，须把对外域名加入白名单，否则会被 421 拒绝；输入支持纯域名 / `host:port` / `host:*` 三种形态（`ipaddress` 判别归一化，IPv6 自动裹方括号）；每个域名自动同时生成 `:*` 端口通配（非标准端口兜底）与无端口精确（默认 443/80 下浏览器不序列化端口）两种 Host 条目，并放行 http/https 双 scheme Origin |
