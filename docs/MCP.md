...
| `--allowed-host` | `CNB_AGENTIC_MEMORY_MCP_ALLOWED_HOSTS` | 无 | DNS rebinding 防护额外放行的 Host 白名单（可多次传入或逗号分隔）。反代按最佳实践保留真实 Host（`proxy_set_header Host $host`）部署时，须把对外域名加入白名单，否则会被 421 拒绝 |
