...
- **错误处理分三类，勿以 `isError` 判成功**：① **业务拒绝**（语义层参数拒绝如 `state=all`、`memory_keyword_search` 空检索词、写路径回读不一致、知识库不可用、`--require-headers` 拒绝匿名请求等）返回**正常结果**（`isError=false`），正文为 `{"error": "…"}` 且含行动指引，须按**顶层 JSON 的 `error` 键**存在性判别（勿对文本做子串/递归查找——成功结果的 `body` 值内可含 `"error"` 字面量）——尤其 `memory_write` 部分落盘时 `error` 字段携带已落盘分片编号，漏判会留孤儿分片。注意 `memory_search` 无空检索词前置校验，其行为随知识库状态分叉（KB 健康时空词返回 `[]` 空结果、KB 故障时落 ①），勿以「空检索词」作判据示例；② **参数 Schema 校验拒绝 / 未知工具**（缺必填字段、类型错、工具名拼错）为 `isError=true`，正文含 `validation error`（Schema 校验，单/多字段错误分别为 `Error executing tool <名称>: 1 validation error for …` / `…: N validation errors for …`）或以 `Unknown tool:` 开头（未知工具，无框架前缀），并含**完整字段级明细（含调用方实参回显）**，可直接改参重试；③ **运行期异常**（上游 API 错误 / 请求期失败 / 未预期错误）为 `isError=true`，经服务端统一出口收敛为**含类别的可判别文案**（如「上游 API 错误（HTTP 502）」「上游响应结构不符预期（HTTP 200 但内容无法解析）」「请求期失败（InvalidURL）」「工具执行失败（<类别>）」），**不含异常明细**——上游响应体等不可信内容在本类出口一律不回显（`ApiError.message` 在构造期已截断至 500 字符；该截断值用于服务端日志，**另经 ① 通道回显，见下方豁免清单**），细节只在服务端日志（`memory_search` 的知识库故障已在内层转为 ①，故其文本保留 CNB 原文；仓库未配置知识库流水线时 `memory_search` 会失败，错误文本提示可改用 `memory_keyword_search`，标题检索无需知识库）。② 与 ③ 共享 `Error executing tool` 前缀：② 的明细在正文里，③ 无明细。**例外——配置缺失不要重试**：**仅在 `isError=true` 且正文**含「缺少必需配置：」且不含 `validation error` / `Unknown tool`（含则属 ②，先按参数问题处理）**时**（凭据未提供或提供不齐，可能来自服务端环境变量或请求头，视部署形态而定），把缺失项清单转达给用户并等其完成配置后重试——不要猜测连接参数，不要编造或代填凭据。`isError=false` 的成功结果（如记忆正文）与 ② 的实参回显都可能含该字面量，勿据内容子串误判。

  **③ 的豁免清单（回显截断后上游原文的唯一两条通道）**：③ 出口本身不回显响应体，但下列两条路径会把 `ApiError.message`（截断至 500 字符）拼入 **① 业务拒绝** 的 `{"error": …}` 正文并以 `isError=false` 返回，属 ① 通道的既有行为、与 base 同源：

  1. `memory_search` 知识库检索失败：`知识库检索失败（ApiError: CNB API <code>: <截断后响应原文>）。可改用 keyword_search…`
  2. `memory_write` 部分落盘失败：`写入失败（已完成 N/M）…原始错误：ApiError: CNB API <code>: <截断后响应原文>`

  判定方式：以 `isError` 取值为准——`isError=true`（③ 类）不含响应明细；`isError=false` 且正文含 `ApiError: CNB API` 时才可能携带。除这两条外，③ 出口一律不回显。
