"""EdgeOne Pages Python 运行时适配层：streamable-http 形态承载 MCP Server。
...
SDK 的 Host/Origin 传输层校验在 EO 形态下不启用，防护职责分层：
...
# 兜底在此路径不生效，开关恒 False——语义与 CLI 完全同源。
# 启动告警落 EO 函数日志：门禁关闭时共享部署匿名可调全部工具，漏配 env 不能
# 零痕迹；请求期门禁由 ensure_require_headers 每请求惰性求解
