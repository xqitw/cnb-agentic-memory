...
cnb-agentic-memory (cam) — 基于 CNB 平台的通用智能体记忆工具。以 CNB Issue 为存储、CNB 知识库为语义检索，以 MCP Server 形式对外提供服务。Python 3.11+，PyPI 包名 `cnb-agentic-memory`，CLI 命令 `cam`，MCP Server 入口 `cam-mcp`。
