...
- **阻塞判定标准**：仅当问题由本 PR diff 引入、或与本 PR 声明的需求/红线直接相关时，才作为阻塞项
- **潜伏问题转 issue**：评审发现的、存在于基线代码的历史问题，一律单独建 issue 跟踪并在评论中给出 issue 链接，不阻塞合并
- **设计岔路口先对齐**：修复一个阻塞项存在多种方案且复杂度差异巨大时，先在评论中对齐方案再写代码，避免逐形态打地鼠
- **修复引入回归单独标注**：修复本身引入的新问题仍属阻塞，但应触发「方案是否需要重选」的检查，而不是继续点对点修补

背景：PR #97 评审循环中范围从 2 项需求膨胀到 10 个提交仍报新阻塞，本节约定由此沉淀（Ref: #102）。
